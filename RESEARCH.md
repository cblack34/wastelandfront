...
- [x] base-building.astro REWRITE — worst page on the site (~90% generic): invents building names ("Troop Center", "Research Lab", "Gear Workshop"; real: Tank/Air/Missile Center, Tech Center, Gear Factory, Drill Ground), zero numbers, no stamp. Needs real HQ-gated building research first — rewritten in [#49](https://github.com/cblack34/wastelandfront/pull/49)
- [x] resources.astro REWRITE (~60–70% generic) — keep the Monica/Doom Elite mechanic (fix name, source the bonus %: 25 vs 39% disputed), correct or remove the Arms Race chest-timing advice (sources say phase-matched, not final-hours hoarding), replace unsourced priority rankings with cited numbers or cut — rewritten in [#50](https://github.com/cblack34/wastelandfront/pull/50)
- [x] src/pages/squad-building/aircraft.astro + missile.astro — either complete as real guides (needs hero-type verification for DVA/Carlie/Schuyler and Tesla/Swift/McGregor first) or strip to honest placeholders that make no unverified type claims — stripped to honest placeholders in [#48](https://github.com/cblack34/wastelandfront/pull/48); full guides blocked on hero-type verification (P2)
- [x] alliance-tools.astro — fix the data-copy vs visible-text mismatches in 3 of 4 templates (copy button copies different text than shown); replace generic filler templates with event-specific ones after Alliance Duel research — copy bug fixed in [#47](https://github.com/cblack34/wastelandfront/pull/47), templates upgraded in [#51](https://github.com/cblack34/wastelandfront/pull/51)

### P2 — external verification gaps (single-source or site-internal-only claims)

- [x] Mono-type formation bonus (+5/10/15/20%) and ±20% type-counter cycle: only ever self-cited between our own pages; already flagged "reverse-engineered only" in §1. Get Tier-1/2 citations or in-game screenshots — Whiteout Survival contamination is the named risk — VERIFIED in [#53](https://github.com/cblack34/wastelandfront/pull/53): bonus table confirmed ×2 Tier-1 (contamination check clean); counter direction confirmed ×2; verified scope: bonus table + counter direction only; still open (tracked in the Inbox in-game item): counter magnitudes beyond Tank↔Missile and the stacking formula
- [x] Re-run the squad-cluster research passes that didn't complete in the audit (squad-building, aircraft, missile, tank external verification; row-targeting; wishlist ~10 shards/200 recruits; 5★ EW/WoH gate) — done in the P2 bundle ([#53](https://github.com/cblack34/wastelandfront/pull/53)): five of six stub-page hero types confirmed ×2 (DVA/Carlie/Schuyler Aircraft; Tesla/McGregor Missile); Swift's Missile type is Tier-2-only so far (Inbox); wishlist corrected to per-200-wish-tickets; EW/WoH gates confirmed
- [x] "Season Hero Swap vouchers" (heroes.astro): highest fabrication suspicion on the site — no footprint anywhere else in the repo; verify it exists or remove — removed in [#38](https://github.com/cblack34/wastelandfront/pull/38) (no verifiable content)
- [x] "Back row center targeted last" (squad-building, tank): zero corroboration for within-row targeting priority — source or cut — REMOVED in [#53](https://github.com/cblack34/wastelandfront/pull/53): traces to Whiteout Survival's Arena mechanic, no Last War source exists
...
- [ ] From the P2 bundle ([#53](https://github.com/cblack34/wastelandfront/pull/53)): does the mono-type 3+2 tier require the 2 off-type heroes to match each other?; Missile→Aircraft and Aircraft→Tank counter magnitudes + additive-vs-multiplicative stacking (combat-log test); Swift's Missile type lacks a Tier-1 hit (lastwar.wiki spot-check); Dielectric Ceramic VIP 6 vs 9 (theriagames says 9; cpt-hedge VIP-store page 403s); "Season Merit Medal" as the likely real store-currency name; Black Market ~600/day cash cap (Tier-3 only; cpt-hedge page 403s)
- [ ] In-game checks from the base-building research (#49): exact HQ15-30 secondary-building gate rotation; Missile/Tank/Air Center unlock levels; Hospital level-by-level capacity; Emergency Center 21%/450 figures; T10 gate composition; "Alliance Center" vs "Alliance Support Hub" in-game name; Material Workshop skip-vs-prioritize dispute
- [ ] In-game checks from the resources/Alliance Duel research (#50/#51): Monica's Treasure Hunter % (39 flat vs ~30.9+5/star); Doom Elite rally stamina cost + reward distribution + Monica stacking; Arms Race exact phase list (single-source); Alliance Duel weekly VP threshold (~7) and chest point thresholds; stamina cap 120 / regen 1-per-300s (Tier-3 only)
